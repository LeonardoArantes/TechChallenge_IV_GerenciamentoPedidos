        ...
        try {
            ...
            for (Produto produto : pedidoDTO.produtos()) {
                ...
                if (produto.getQuantidade() > item.quantidade()) {
                    throw new ResourceNotFoundException("Produto sem saldo para a venda: " + produto.getProdutoId() + " Saldo: " + produto.getQuantidade());
                }
            ...
            throw new UnsupportedOperationException("Não foi possível gerar o pedido!"); // IllegalArgumentException
        }
    }
